...
namespace sortTestHelper
{
    ...
    int *genRandomArr(int from, int to, int amount)
    {
        ...
        for(int i = 0; i < amount; i++)
        {
            ret[i] = rand() % (from - to + 1) + from;
        }
        return ret;
    }

    int *genNearlySortedArr(int amount, int swap_time)
    {
        int *ret = new int[amount];
        srand(time(NULL));
        for(int i = 0; i < amount; i++)
    ...
    void sortBenchMark(const string name, void (*fun)(T *, int), T *arr, int amount)
    {
        ...
        cout << name << " Algorithms spend times is: " << (double)(end_time - start_time) / CLOCKS_PER_SEC << " s " << endl;
    }
}
